  ...
  try {
    ...
    if (user) {
      ...
      if (credits) {
        ...
        if (remainingCredits < credits.total_credits * 0.1) {
          logger.warn(`User ${user.id} running low on credits: ${remainingCredits} pence remaining out of £${(credits.total_credits / 100).toFixed(2)}`)
        }
      }
    }
    ...
    if (user) {
      try {
        ...
        logger.warn('Failed to fetch learning preferences:', error)
