 ...
 * Uses client-side browser client for all operations
 */
...
export class SupabaseDatabaseService {
  ...
  static async saveChatSession(
    ...
    try {
      const sessionData: NewChatSession = {
        id: sessionId,
        title,
        messages: messages as any, // Supabase JSONB type
        model,
        user_id: userId || null,
      ...
      if (!data) {
        throw new Error('No data returned from chat session save');
      }

      console.log(`✅ Saved chat session: ${sessionId}`);
      return data;
    } catch (error) {
  ...
  static async saveChatSummary(
    ...
    try {
      const summaryData: NewChatSummary = {
        ...
        user_preferences: userPreferences as any, // Supabase JSONB type
      ...
      if (!data) {
        throw new Error('No data returned from chat summary save');
      }

      console.log(`✅ Saved chat summary: ${summaryId}`);
      return data;
    } catch (error) {
  ...
  static async getSessionsWithSummaries(userId?: string, limit = 20): Promise<Array<{
    ...
    if (error) {
      console.error('Failed to get sessions with summaries:', error);
      throw new Error(`Database error: ${error.message}`);
    }
    ...
    return (data || []).map((item: any) => ({
      ...
      summary: item.chat_summaries?.[0] || null,
    }));
  }
  ...
  static async getUserActivity(userId: string): Promise<{
    ...
    const activeChats = allSessions.filter(s => s.is_active).length;
    ...
    const recentActivity = allSessions.slice(0, 10);
